...
class StockFilters:
    ...
    def get_wide_columns(self):
        ...
        merged_df = pd.merge(ticker_stock_and_recored_on, mission_two_df, how="left")
        merged_df = pd.merge(merged_df, mission_three_df, how="left")
        merged_df = pd.merge(merged_df, mission_four_df, how="left")
        merged_df = pd.merge(merged_df, mission_five_df, how="left")
        merged_df = pd.merge(merged_df, mission_six_df, how="left")
        merged_df = pd.merge(merged_df, mission_seven_df, how="left")
        merged_df = pd.merge(merged_df, mission_eight_df, how="left")
        merged_df = pd.merge(merged_df, mission_nine_df, how="left")
        merged_df = pd.merge(merged_df, mission_ten_df, how="left")
        return merged_df
